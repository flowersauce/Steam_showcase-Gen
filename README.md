...
![Code Style](https://img.shields.io/badge/Code_Style-LLVM-black.svg?style=flat-square&logo=llvm)
...
* **多格式支持**：支持视频 (`.mp4`, `.avi`, `.mov`, `.mkv`) 和图片 (`.png`, `.jpg`, `.webp`) 输入。
* **自动切片**：自动调整比例并切割为 Steam 展柜标准的 5 张切片 (150px 宽 + 4px 间隙)。
* **Hex Hack 集成**：自动修改 GIF 文件尾部字节 (`0x3B` -> `0x21`)，利用 Steam 漏洞实现“无限高度”及防止压缩。
...
```bash
# 1. 克隆仓库
git clone [https://github.com/flowersauce/Steam_showcase-Gen.git](https://github.com/flowersauce/Steam_showcase-Gen.git)
